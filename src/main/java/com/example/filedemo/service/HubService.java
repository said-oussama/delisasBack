...
public Hub retrieveHub(Long id) {
	if (id == null) {
		throw new IllegalArgumentException("L'ID du hub ne doit pas être null.");
	}

	Hub hub = hubrepository.findById(id).orElseThrow(() -> new HubNotFoundException("Hub avec l'identifiant " + id + " non trouvé."));
	return hub;
}
