...
import com.example.filedemo.exception.HubNotFoundException;
...
//		Hub hubArrivee = hubService.retrieveHub(consolePayload.getIdHubArrivee());
//		Hub hubDepart = hubService.retrieveHub(consolePayload.getIdHubDepart());
//
//		if (hubArrivee == null) {
//			throw new HubNotFoundException("Hub with ID " + consolePayload.getIdHubArrivee() + " not found.");
//		}
//
//		if (hubDepart == null) {
//			throw new HubNotFoundException("Hub with ID " + consolePayload.getIdHubDepart() + " not found.");
//		}
...
public Console addConsole(ConsolePayload consolePayload) {
	...
	// Vérifier si les hubs existent avant de continuer
	Hub hubArrivee = hubService.retrieveHub(consolePayload.getIdHubArrivee());
	Hub hubDepart = hubService.retrieveHub(consolePayload.getIdHubDepart());

	if (hubArrivee == null) {
		throw new HubNotFoundException("Hub avec l'identifiant " + consolePayload.getIdHubArrivee() + " non trouvé.");
	}

	if (hubDepart == null) {
		throw new HubNotFoundException("Hub avec l'identifiant " + consolePayload.getIdHubDepart() + " non trouvé.");
	}
